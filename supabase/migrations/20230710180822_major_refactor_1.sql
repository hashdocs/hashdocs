    ...
    FROM (
        SELECT
                    ...
                    SELECT
                        tbl_links.*, coalesce((
                            ...
                            FROM (
                                ...
                                INNER JOIN tbl_document_versions ON tbl_views.document_version = tbl_document_versions.document_version AND tbl_document_versions.document_id = tbl_links.document_id
                            WHERE
                                tbl_links.link_id = tbl_views.link_id
                                GROUP BY tbl_views. view_seq, tbl_views.view_id, tbl_document_versions.page_count) sub_views), '[]') AS views
                FROM tbl_links
                WHERE
            ...
            coalesce((
                ...
                FROM 
                    (SELECT tbl_document_versions.* FROM tbl_document_versions WHERE tbl_document_versions.document_id = tbl_documents.document_id) AS sub_versions), '[]') AS versions
        FROM
            tbl_documents
        WHERE
